...
namespace map
{
    public class Utilities
    {
        ...
        public static void LoadTranslations(Type locStringRoot, string modPath, string translationsDir = "translations")
        {
            ...
            Debug.Log(string.Format("Loading translation file for {0} ({1}) language: '{2}'", locale.Lang, locale.Code, text));
            if (!File.Exists(text))
            {
                Debug.LogWarning("Translation file not found: '" + text + "'");
                return;
            }
            try
            {
            ...
            {
                Debug.LogError("Unexpected error while loading translation file: '" + text + "'");
                Debug.LogError(obj);
            }
        }
